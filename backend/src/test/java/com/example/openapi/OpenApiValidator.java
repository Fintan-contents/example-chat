...
public class OpenApiValidator {
    ...
    public void validateRequest(String operationId, HttpRequest request) throws ValidationException {
        ...
        DefaultRequest.Builder requestBuilder = new DefaultRequest.Builder(requestUri, method)
                .headers(convertType(request.headers()));
        int queryIndex = requestUri.indexOf("?");
        if (queryIndex != -1) {
            requestBuilder.query(requestUri.substring(queryIndex + 1));
        }
        ...
        if (request.getMethod() == Request.Method.GET) {
            ...
            if (query != null) {
                ...
                if (queryIndex != -1) {
                    ...
                    List<String> queryParameterNames = Arrays.stream(queryParameters)
                            .map(queryParameter -> queryParameter.split("=")[0]).collect(Collectors.toList());
