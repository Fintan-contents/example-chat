...
public class CsrfTokenActionIT extends SimpleRestTestSupport {
    ...
    public void CSRFトークンを取得できる() throws Exception {
        ...
        context.setSessionStoredVar(WebConfigFinder.getWebConfig().getCsrfTokenSessionStoredVarName(), "123");
        ...
        JsonAssert.with(response.getBodyString())
                ...
                .assertEquals("$.csrfTokenHeaderName", WebConfigFinder.getWebConfig().getCsrfTokenHeaderName())
                .assertEquals("$.csrfTokenParameterName", WebConfigFinder.getWebConfig().getCsrfTokenParameterName());
    }
}
