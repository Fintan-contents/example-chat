package com.example.system.nablarch.log;

import java.util.Map;
import java.util.Map.Entry;
import java.util.stream.Collectors;

import com.example.system.nablarch.threadcontext.HttpMethodAttribute;
import com.example.system.nablarch.threadcontext.SessionIdAttribute;
...
public class ExtendedLogFormatter extends BasicLogFormatter {
    ...
    protected Map<String, LogItem<LogContext>> getLogItems(ObjectSettings settings) {
        ...
        return logItems.entrySet().stream().collect(Collectors.toMap(
                Entry::getKey, e -> new NullToHyphenLogItemWrapper(e.getValue())));
    }
